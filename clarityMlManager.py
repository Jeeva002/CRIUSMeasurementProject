...
async def extract_measurements(request: MeasurementExtractRequest):
    ...
    try:
        ...
        # if not success:
        #     raise HTTPException(status_code=404, detail="Failed to download DICOM file")
        result1, result2 = processDicom(str(folderPath))
        # match your documented response structure (list of dicts)
        return [result1, result2]
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error in measurement extraction: {str(e)}")
